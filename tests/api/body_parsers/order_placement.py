...
class OrderPlacementParserTestCase(unittest.TestCase):

    def setUp(self):
        self.patches = []
        self.mocks = {}

        flask_parser_patch = patch('orders.api.body_parsers.order_placement.reqparse')
    ...
    def test_init_calls_create_parser_and_parse_args(self):
        ...
        # Act
        OrderPlacementParser.__init__(mock_self)
    ...
    def test_create_parser_returns_parser_with_all_fields(self):
        ...
        mock_self = MagicMock(
            _fields=[
                {'name': 'test', 'type': str, 'location': 'form', 'required': True},
                {'name': 'test123', 'type': str, 'location': 'form', 'required': True}
            ]
        )
        expected_calls = [
            call(
        ...
        # Act
        OrderPlacementParser._create_parser(mock_self)

        # Assert
        self.mocks['flask_parser_mock'].RequestParser.return_value.\
